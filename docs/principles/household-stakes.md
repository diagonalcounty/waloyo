# Stakes — you have each other

Issue: [#25](https://github.com/diagonalcounty/waloyo/issues/25).  
Stance: [compassionate-self-interest.md](compassionate-self-interest.md).

The town is the field. The **pressure** is the other person.

If you are playing the man, you already know a season is coming when she cannot stand at the stall the way she does now. She will bear a child and take primary responsibility for raising it. That is not a random event card. It is a **foreseeable labor shock**. Childrearing is work. What stalls is *market* work — walking, carrying, bargaining, a full day’s trade.

Wealth is how the household stays intact through that season without eating the neighbors or disappearing.

---

## Card: `household-coming-season`

**Name:** Build for the season only one of you can work the market.

**Source:** Project. Kin to household economics, Hayek’s adaptation to change, stewardship.

**Mechanism:** A two-worker household that spends like a two-worker household will break when it becomes one-plus-a-child. Surplus, a skill that still pays, and neighbors who will still trade with you are how care becomes possible. The child is the reason the trade has to *last*, not a decoration.

**Player does:** Choose work and stores that still feed two (then three) when she is home with the infant. Prefer a deal the town will repeat without her hands on the cart.

**Player fails:**
- *Selfish:* spend the surplus on himself, or treat her as a cost to minimize. The season arrives; the pot does not.
- *Selfless:* give the stock away while both can still walk. The town remembers the gift. The child does not eat memory.
- *Ignore the clock:* live week to week as if both will always make market day. Hayek’s locked recipe; the world moved inside the house.

**Chapters:** open (repeats in every era the couple is a fertile household)

---

## What this does to play

- The clock is intimate, not military. Warcraft’s “something is coming” is a pregnancy and a winter, not an orc wave.
- Compassionate self-interest is no longer abstract kindness. It is *get a trade that still works when she cannot come*.
- The town still matters: a tanner’s wage, a standing customer, a stored sack — things that persist when one pair of hands is busy with the child.

The market-morning prototype had none of this. That is why it had no stakes.
